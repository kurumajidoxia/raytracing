...
void raytracing(void *raytracing_p)
{
    point3 u, v, w, d;
    color object_color = { 0.0, 0.0, 0.0 };
    raytracing_para *para=raytracing_p;
    ...
    for (int j = para->height_begin; j < para->height_end; j++) {
        for (int i = para->width_begin; i < para->width_end; i++) {
            double r = 0, g = 0, b = 0;
            /* MSAA */
            for (int s = 0; s < SAMPLES; s++) {
                ...
                if (ray_color((para->view)->vrp, 0.0, d, &stk, para->rectangulars, para->spheres,
                              para->lights, object_color,
                ...
                para->pixels[((i + (j * ROWS)) * 3) + 0] = r * 255 / SAMPLES;
                para->pixels[((i + (j * ROWS)) * 3) + 1] = g * 255 / SAMPLES;
                para->pixels[((i + (j * ROWS)) * 3) + 2] = b * 255 / SAMPLES;
            }
        }
    }
    pthread_exit(0);
